...
tex-fmt --reflow minimal file.tex  # reflow with minimal line changes
...
\subsection{Multiline optional arguments}

With \texttt{--format-options}, each top-level comma-separated item in an
already multiline command option is put on its own indented line. Single-line
options remain compact, and nested braces or brackets remain part of their
surrounding item. Options containing comments or items which themselves span
lines are left unchanged because rewriting them could alter meaningful
whitespace.
...
\subsection{Paragraph reflow}

Paragraph reflow is disabled by default. \texttt{wraplen} is the hard
line-length limit, while \texttt{wrapmin} is the target used when choosing a
break rather than a strict minimum. With \texttt{--reflow minimal}, an existing
break is stable when its line has reached \texttt{wrapmin}, or when pulling the
next legal text chunk would reach or cross that target. Text is redistributed
only around boundaries which have not reached this equilibrium or whose lines
exceed \texttt{wraplen}, which keeps version-control diffs small after prose
edits.

With \texttt{--reflow canonical}, eligible paragraph line breaks are removed
before wrapping, producing a consistent layout without preferring the input
boundaries.

With \texttt{--reflow semantic}, every existing line break is kept and a new
break is added after each sentence, approximating one sentence per line. A
period ends a sentence only when it is followed by a space and is not part of a
decimal, an initial, or a common abbreviation such as \texttt{e.g.} or
\texttt{Fig.}. A sentence still longer than \texttt{wraplen} is broken at clause
boundaries (\texttt{,}, \texttt{;}, \texttt{:}), never inside math, braces,
brackets, or parentheses, before the ordinary wrapping pass handles anything
that remains too long.

Comments, displayed mathematics, tables, verbatim content, and other protected
regions are not reflowed. Reflow is disabled when wrapping is disabled.
...
\begin{table}[H]
  \begin{center}
    \begin{tabular}{|l|l|l|l|}
      ...
      \texttt{--reflow <MODE>}      &       & \texttt{off}   &
      Paragraph reflow strategy: \texttt{off}, \texttt{minimal},
      \texttt{canonical}, or \texttt{semantic} \\
      \texttt{--format-options}     &       &         & Format
...
\begin{table}[H]
  \begin{center}
    \begin{tabular}{|l|l|l|l|}
      ...
      & Soft target for line breaks \\
      \texttt{reflow}         & str      & \texttt{"off"},
      \texttt{"minimal"} & Paragraph reflow strategy; also \\
                            & & & accepts \texttt{"canonical"} and
      \texttt{"semantic"} \\
      \texttt{format-options} & bool     & \texttt{false}
      & Format multiline optional arguments \\
