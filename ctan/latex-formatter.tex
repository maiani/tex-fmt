...
Paragraph reflow is disabled by default. With
\texttt{--reflow minimal}, existing line breaks are kept whenever their lines
are between \texttt{wrapmin} and \texttt{wraplen}, unless moving a nearby
break is required to satisfy those limits. Text is redistributed across the
smallest feasible surrounding region when a line falls outside the range,
which keeps version-control diffs small after prose edits.
